...
def a_star(start,end,present):
    ...
    for v in graph1[start]:
        cost=(v,hsld[v]+graph2[start][v]+present)
        cost_list.append(cost)
        cost_sorted=sorted(cost_list, key=lambda x: x[1])
    a_star(cost_sorted[0][0],end,graph2[start][cost_sorted[0][0]]+present)
a_star("Arad","Bucharest",0)
